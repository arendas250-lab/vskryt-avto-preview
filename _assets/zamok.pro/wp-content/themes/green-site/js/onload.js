// window.onload = function() {
  ...
  function google_mainMap_lazyload(api_key) {
    'use strict'
    ...
    if (api_key) {
      ...
      var observer = new IntersectionObserver(
        function(entries, self) {
          ...
          if (isIntersecting) {
            var mapsJS = document.createElement('script')
            mapsJS.src = 'https://maps.googleapis.com/maps/api/js?callback=initMainMap&key=' + api_key
            document.getElementsByTagName('head')[0].appendChild(mapsJS)
            self.unobserve(map);
          }
        },
  ...
  document.getElementById('mainMap') && google_mainMap_lazyload("AIzaSyDgrPVjPvlC_sBmihsgxAdwcpjOu1OIhWc");
